export const questions = [
  {
    id: "vibe",
    label: "What’s your favorite holiday vibe?",
    options: ["Heartwarming", "Hilarious", "Magical", "Action-packed"],
  },
  {
    id: "movieStyle",
    label:
      "Are you in the mood for a classic holiday movie or something more modern?",
    options: ["Classic", "Modern"],
  },
  {
    id: "holidayType",
    label:
      "Do you prefer movies set around Christmas, Hanukkah, or just winter in general?",
    options: ["Christmas", "Hanukkah", "Winter"],
  },
  {
    id: "ageType",
    label:
      "Do you want a romantic holiday story or something focused on family and friendship?",
    options: ["Romantic", "Family and Friendship"],
  },
  {
    id: "ageType",
    label:
      "Do you want a romantic holiday story or something focused on family and friendship?",
    options: ["Romantic", "Family and Friendship"],
  },
  // 2. Themes and Genres
  {
    id: "storyType",
    label:
      "Do you want a romantic holiday story or something focused on family and friendship?",
    options: ["Romantic", "Family and Friendship"],
  },
  {
    id: "movieCharacters",
    label:
      "Would you enjoy a movie about Santa, snowmen, or magical adventures?",
    options: ["Santa", "Snowmen", "Magical Adventures"],
  },
  {
    id: "movieGenre",
    label:
      "Are you looking for a musical, a comedy, a drama, or even a holiday thriller?",
    options: ["Musical", "Comedy", "Drama", "Holiday Thriller"],
  },

  // 3. Audience
  {
    id: "audienceType",
    label: "Are you watching alone, with kids, or with a group of adults?",
    options: ["Alone", "With Kids", "With Adults"],
  },
  {
    id: "movieRating",
    label:
      "Do you want something kid-friendly, family-oriented, or a little more grown-up?",
    options: ["Kid-Friendly", "Family-Oriented", "Grown-Up"],
  },

  // 4. Setting and Mood
  {
    id: "holidaySetting",
    label:
      "Do you prefer a cozy small-town holiday or a bustling big-city Christmas?",
    options: ["Cozy Small Town", "Bustling Big City"],
  },
  {
    id: "holidayMood",
    label:
      "Are you into snowy, magical settings, or something a bit more tropical and unconventional?",
    options: ["Snowy", "Magical", "Tropical", "Unconventional"],
  },

  // 5. Favorites and Nostalgia
  {
    id: "holidayTradition",
    label:
      "Do you have a favorite holiday tradition or character you’d like to see in the movie?",
    options: ["Tradition-Based", "Character-Based"],
  },
  {
    id: "movieNewOrOld",
    label:
      "Do you want a movie you've seen before or something completely new?",
    options: ["Seen Before", "Something New"],
  },

  // 6. Time Commitment
  {
    id: "timeCommitment",
    label:
      "How much time do you have: a quick hour-long special, a standard movie, or a full marathon?",
    options: ["Quick Special", "Standard Movie", "Full Marathon"],
  },

  // 7. Quirky Fun Questions
  {
    id: "holidayRescue",
    label:
      "Which would you rather: rescuing Christmas, saving the family dinner, or winning a holiday contest?",
    options: [
      "Rescuing Christmas",
      "Saving Family Dinner",
      "Winning a Holiday Contest",
    ],
  },
  {
    id: "holidayTreat",
    label:
      "What’s your favorite holiday treat: hot cocoa, gingerbread, candy canes, or fruitcake?",
    options: ["Hot Cocoa", "Gingerbread", "Candy Canes", "Fruitcake"],
  },
  {
    id: "holidayCompanion",
    label:
      "If you could spend Christmas with anyone, would it be Santa, a snowman, or a reindeer?",
    options: ["Santa", "Snowman", "Reindeer"],
  },
];
